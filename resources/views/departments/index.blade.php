    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-gradient-to-l from-[#fb7185] via-[#a21caf] to-[#6366f1] rounded-xl p-4 border border-purple-500">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-white font-medium">Disabled</p>
                        <p class="text-2xl font-bold text-white">{{ $departments->where('status', 'disabled')->count() }}</p>
                    </div>
                        ...
                        <tr class="hover:bg-gradient-to-r hover:from-gray-50 hover:to-transparent transition-all duration-200 group">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                <div class="flex items-center justify-center">
                                    <form action="{{ $department->status == 'enabled' ? route('departments.disable', $department) : route('departments.enable', $department) }}" 
                                          method="POST" 
                                          class="inline toggle-form"
                                          id="toggle-form-{{ $department->id }}">
                                        @csrf
                                        <label class="relative inline-flex items-center cursor-pointer">
                                            <input type="checkbox" 
                                                   class="sr-only peer toggle-checkbox" 
                                                   data-form-id="toggle-form-{{ $department->id }}"
                                                   {{ $department->status == 'enabled' ? 'checked' : '' }}>
                                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600"></div>
                                            <span class="ml-3 text-sm font-medium text-gray-700" id="status-label-{{ $department->id }}">
